...
const EventTypeModuleOwnerRemoved = "protoradar.module_owner.removed"

type ModuleOwnerRemovedPayload struct {
...
type ModuleOwnerRemoved struct {
	Owner      domain.ModuleOwner
	Actor      string
	OccurredAt time.Time
}

func NewModuleOwnerRemoved(event ModuleOwnerRemoved) (outbox.Record, error) {
	occurredAt := event.OccurredAt
	if occurredAt.IsZero() {
		occurredAt = event.Owner.UpdatedAt
	}
	payload := ModuleOwnerRemovedPayload{
	...
	return outbox.Record{
		...
		EventType:     EventTypeModuleOwnerRemoved,
		DedupKey:      fmt.Sprintf("module:%s:owner:%s:removed", moduleID, ownerID),
		Payload:       body,
		OccurredAt:    occurredAt,
	}, nil
}
